...
{
    _upMWCDModel.reset( new MWCDModel(
...
void MainWindow::on_bt_startDraw_clicked()
{
    ...
    if( _upMWCDModel.get()->paramsHasZero() )
    {
    ...
    if( ui->rb_drawBreakthroughTime->isChecked() )
    {
        ...
        resultTuple = _upMWCDModel.get()->getBTPredicionData();

    }
    else if( ui->rb_drawInputTime->isChecked() )
    {
        qDebug() << "rb_drawInputTime: " << ui->lineEdit_time->text();
        resultTuple = _upMWCDModel.get()->getITPredicionData( ui->lineEdit_time->text().toDouble() );
    }
    ...
    double legendNum = ( static_cast<int>( _upMWCDModel.get()->getBreakthroughTime() * 10 + 0.5 ) ) / 10.0;
    qDebug() << legendNum;
    ...
    MWCDModel::printModelParams( *_upMWCDModel.get() );
    qDebug() << MWCDTool::doubleToString( _upMWCDModel.get()->getBreakthroughTime() ).c_str();
...
{
    QString time( QString::fromLocal8Bit( "见水时间 t = %1年" ).arg( _upMWCDModel.get()->getBreakthroughTime() ) );
...
{
    initModel();
    _pSAWindow = new SensitivityAnalysis( *_upMWCDModel );
    _pSAWindow->show();
}
